...
class NumericStack extends LengthLimitedStack<number | null> {
    ...
    popPlaceholder(): void {
        if (this.pop() !== null) throw new Error("Popping placeholder but found non-null value");
    }
    popNumber(): number {
        const x = this.pop();
        if (x === null) throw new Error("Popping number but found null");
        return x;
    }
        ...
        for (; iter.isInBound(); iter.next()) {
            ...
            if (Object.values(literalTokenTypes).includes(cur.type)) {
                ...
                if (expectNumber) {
                    ...
                    if (cur.type === allTokenTypes.neg) throwSyntax(iter, "Negative sign cannot follow a number (ommitted multiplication excludes negative sign)");
                    evalStacks.evalUntil(iter, Precedence.L4);
                    evalStacks.command.push({
